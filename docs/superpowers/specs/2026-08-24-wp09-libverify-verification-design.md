...
`opus_corpus.libverify` owns the native boundary. `LibverifyVerifier` loads an explicitly supplied `libverify` shared library, hashes the exact library bytes, drives the documented byte-array verifier API, and returns the existing `VerificationResult` contract. A narrow backend protocol isolates FFI mechanics from result mapping so hermetic tests can exercise verification semantics without compiling native code.

`opus_corpus.verification_materialization` owns deterministic conversion from canonical artifact records to canonical verification records. It reads exact bytes through the existing `ContentStore`, matches each solution artifact to the unique canonical puzzle artifact for its `puzzle_id`, invokes a `Verifier`, and returns verification records in deterministic order. It creates no second object store or verification authority.

## Verifier identity and validation profile

The implementation identity is `omsim-libverify`.

The verifier revision is the pinned `omsim` commit above. `verifier_sha256` is the SHA-256 of the exact loaded shared-library file. The hash is runtime evidence rather than a repository-wide constant because native binary bytes depend on the build platform and toolchain.
...
If libverify reports a puzzle-file or solution-file error immediately after creation, the result records `parse_status = failed`, `simulation_status = not_run`, null metrics, and a stable error code (`puzzle_parse_failed` or `solution_parse_failed`).

Once creation succeeds, `parse_status = passed`. Metric evaluation drives simulation. Any libverify error during metric evaluation records `simulation_status = failed`, nulls all four authoritative metrics, and retains a stable error code plus deterministic detail. Successful evaluation records `simulation_status = passed` and the four recomputed integer metrics.
...
Repeated evaluation of identical artifact bytes, exact shared-library bytes, pinned revision, and validation profile must produce identical `VerificationResult` values.

Materialization rejects missing puzzle artifacts, more than one puzzle artifact for a solution's puzzle, unsupported artifact kinds/formats, corrupt/missing content-store objects, unsupported validation profiles, and native API errors. Artifact iteration order and cache-root location must not affect logical output.

## Native-library provisioning

WP-09 does not introduce a verifier download service, source cache, or vendored copy of `omsim`. The shared library is an explicit runtime input built from the pinned upstream revision. The repository's upstream contract test downloads that exact revision in CI, builds `libverify.so` with the upstream source set, and exercises the real FFI path.

## Testing

Hermetic tests cover profile rejection, binary hashing, success mapping, puzzle/solution parse failures, simulation/metric failures, handle cleanup, deterministic repeated results, artifact pairing, object integrity, and input-order independence using a scripted backend and fake verifier where appropriate.
